    ...
    const fetchEmployees = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Log this to your browser console to see the exact keys!
      ...
      <div className="max-w-4xl mx-auto pt-12 px-6">
        <div className="mb-10">
          ...
          <p className="text-muted-foreground mt-2 font-medium">
            Define the unit name, capacity, and commander.
          </p>
        </div>

        <form
          onSubmit={handleSubmit}
          className="space-y-8 bg-card/40 p-8 rounded-[2.5rem] border border-border shadow-2xl backdrop-blur-md"
        >
...
>
  ...
  {employees.map((emp, index) => {
    // 1. Try to find the ID in any common format
    const id = emp.empId || emp.EMP_ID || emp.id || emp.employeeId;
    
    // 2. Try to find the names
    const firstName = emp.firstName || emp.EMP_FIRSTNAME || emp.first_name || "Unknown";
    const lastName = emp.lastName || emp.EMP_LASTNAME || emp.last_name || "";

    return (
      /* Using index + id ensures the key is ALWAYS unique even if the API data is messy */
      <option key={`emp-${id}-${index}`} value={id || ""}>
        {firstName} {lastName} {id ? `(ID: ${id})` : "(No ID found)"}
      </option>
    );
  })}
</select>
          ...
          <div className="flex justify-between items-center pt-6 border-t border-border">
            ...
            <Button
              type="submit"
              className="h-12 px-10 rounded-2xl font-bold shadow-lg shadow-primary/20 bg-primary"
            >
              Deploy Squad
            </Button>
          </div>
